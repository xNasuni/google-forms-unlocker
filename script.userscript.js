...
IntervalId = setInterval(() => {
    ...
    if (document.originalAEL == undefined) {
        document.originalAEL = document.addEventListener;
    }
    document.addEventListener = function(eventType, method, options) {
        console.log(`type ${eventType} registered with`, method)
        if (eventType == 'mozvisibilitychange' || eventType == 'webkitvisibilitychange' || eventType == 'msvisibilitychange' || eventType == 'visibilitychange') {
            ...
            return document.originalAEL.apply(this, arguments)
        }
    }
...
(async () => {
    if (typeof chrome === "undefined") { // Spoof Chrome Extension API
        ...
        window.chrome.runtime.ContextType = {
    ...
    function HookFunc(FuncParent, FuncName, HookCallback) {
        (function(OriginalFunction) {
            FuncParent[FuncName] = function() {
                ...
                } else { // use chrome.runtime.sendMessage as normal, ---!!-IMPORTANT-!!---, for some reason, any extension id will be spoofed to return hook callback and i havent figured out why, but this doesn't break anything for now so i will not worry about it
					console.log(`normal`, OriginalFunction, this, arguments)                  
    ...
    function HandleSMHook(HookData) {
        ...
        if (ExtId != "gndmhdcefbhlchkhipcnnbkcmicncehk") {
            return nothing;
        }
        console.warn(ExtId, HookData)
